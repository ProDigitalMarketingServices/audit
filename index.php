...
function audit_build_queue($page){
  $output = false;

  $processed = false;
  $to_process = audit_get_internal_links($page);

  unset($to_process[$page['url']]);

  foreach($to_process as $url => $link){
    if(!in_array($url, $processed)){
      $current_page = audit_get_page($url);

      $current_internal_links = audit_get_internal_links($current_page);

      $output[$url] = array(
        'page' => $current_page,
        'internal_links' => $current_internal_links
      )

      $processed[] = $url;

      $to_process = array_merge($to_process, $current_internal_links);
    }
  }
